...
export default async function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: project }, { data: messages }, { data: files }] = await Promise.all([
  ...
  if (!project) notFound();
  return <ProjectDetail project={project} initialMessages={messages ?? []} initialFiles={files ?? []} userId={user.id} />;
}
