...
export default function PatchDigitizingPage() {
  return <PageLayout>
    ...
    <section className="bg-[#171717] py-20 text-white"><div className="mx-auto max-w-7xl px-4"><div className="mx-auto max-w-2xl text-center"><span className="text-xs font-bold uppercase tracking-[.2em] text-white/45">Why patch digitizing matters</span><h2 className="mt-3 text-3xl font-black sm:text-4xl">A patch needs more than a clean picture.</h2><p className="mt-4 leading-7 text-white/55">The right border, density, color count, and backing plan determine how a patch looks and holds up after production. We prepare every file with the finished patch in mind.</p></div><div className="mt-12 grid gap-5 md:grid-cols-3">{["Clean borders and satin edges", "Balanced stitch density and fills", "Color planning for repeatable production"].map((item) => <div key={item} className="rounded-2xl border border-white/10 bg-[#202020] p-6"><CheckCircle className="h-5 w-5 text-[#d9ff53]" /><h3 className="mt-6 font-black text-white">{item}</h3><p className="mt-2 text-sm leading-6 text-white/50">Practical artwork decisions that help your manufacturer achieve a consistent sew-out or patch finish.</p></div>)}</div></div></section>
