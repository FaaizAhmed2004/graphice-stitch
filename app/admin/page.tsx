...
export default async function AdminPortalPage() {
  ...
  return <main className="min-h-screen bg-[#f5f5f3] text-gray-950">
    ...
    <div className="mx-auto max-w-7xl px-5 py-10"><div className="mb-8 grid gap-4 sm:grid-cols-3"><div className="rounded-2xl bg-gray-950 p-5 text-white"><p className="text-xs font-bold uppercase tracking-wider text-gray-400">New leads</p><p className="mt-3 text-3xl font-black">{leads?.filter((lead) => lead.status === "new").length ?? 0}</p></div><div className="rounded-2xl border border-gray-200 bg-white p-5"><p className="text-xs font-bold uppercase tracking-wider text-gray-400">Open projects</p><p className="mt-3 text-3xl font-black">{projects?.filter((project) => project.status !== "completed" && project.status !== "archived").length ?? 0}</p></div><div className="rounded-2xl border border-gray-200 bg-white p-5"><p className="text-xs font-bold uppercase tracking-wider text-gray-400">CMS pages</p><p className="mt-3 text-3xl font-black">{pages?.length ?? 0}</p></div></div><AdminWorkspace leads={leads ?? []} projects={projects ?? []} clients={clients ?? []} pages={pages ?? []} /></div>
  </main>;
}
