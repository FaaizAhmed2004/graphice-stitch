...
export default function AdminWorkspace({ leads: initialLeads, projects: initialProjects, clients, pages: initialPages, adminUserId }: Props) {
  ...
  useEffect(() => {
    ...
    async function loadProjectFiles() {
      const { data, error } = await supabase.from("project_files").select("id, name, storage_path, file_type, created_at").eq("project_id", selectedProjectId).order("created_at", { ascending: false });
      if (error) setProjectNotice(error.message);
      else setProjectFiles(data ?? []);
    }
    loadProjectFiles();
  ...
  async function openProjectFile(file: ProjectFile) {
    ...
    if (error || !data?.signedUrl) return setProjectNotice(error?.message || "Could not open file.");
